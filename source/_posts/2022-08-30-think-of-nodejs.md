...
用于优先使用本地 node_modules 下的可执行模块,比如全局环境变量下的 webpack 脚本命令版本是 5.x,本地下载的 node_modules 下的
webpack 可执行模块脚本命令版本是 4.x,会优先使用本地下载的 node_modules 下的可执行模块脚本命令版本
...
众多的设计模式里面,最重要的两个设计模式: 观察者模式与外观模式

> 观察者模式

- NodeJS: EventEmitter
- ES5: addEventListener
- ES6: Promise

> 外观模式

- jQuery: element.on('click', () => {}) 其内部逻辑实际是对于各种各类浏览器的兼容

#### 设计原则

六大设计原则中,最重要的两个设计原则: 单一职责原则与开放封闭原则

> 单一职责原则

使用 CommonJS 模块化导入导出的石头、剪刀与布的小程序,可以使用命令行调用,也可以在 HTTP 网页程序中使用

> 开放封闭原则

经典 Webpack 中 loader 的实例,style-loader -> css-loader -> less-loader
